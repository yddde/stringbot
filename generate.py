...
ERROR_MESSAGE = "Oops! An exception occurred! \n\n**Error** : {} " \
            "\n\nTolong Laporan ke @YDDDE jika eror " \
...
async def main(_, msg):
    await msg.reply(
        "Please press the string which you want to take",
...
async def generate_session(bot, msg, telethon=False):
    ...
    try:
        ...
        await api_id_msg.reply('- API_ID not true(which must be an integer). Please start generating session again.', quote=True, reply_markup=InlineKeyboardMarkup(Data.generate_button))
        return
    api_hash_msg = await bot.ask(user_id, 'Please send `API_HASH`', filters=filters.text)
    if await cancelled(api_id_msg):
        return
    api_hash = api_hash_msg.text
    phone_number_msg = await bot.ask(user_id, 'Now send yout Phone_Number with code number.\nExample: `+628xxxxxxx`', filters=filters.text)
    if await cancelled(api_id_msg):
        return
    ...
    text = "**{} STRING SESSION** \n\n`{}` \n\nGenerated by @TeStrSeBOT".format("TELETHON" if telethon else "PYROGRAM", string_session)
    await client.send_message("me", text)
    await client.disconnect()
    await phone_code_msg.reply("Successfully take {} string session. Please check in saved message!  **@YDDDE** .".format("telethon" if telethon else "pyrogram"))
...
async def cancelled(msg):
    if "/cancel" in msg.text:
        ...
        await msg.reply("Canceling Generation Process!", quote=True)
        return True
    else:
        return False
